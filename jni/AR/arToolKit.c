...
//END IMPORTS
/**
 * DetectするMarkerの数。　増やしたいときはこことARToolkit.javaのMAX_MARKERを弄ること。
 */
 ...
 *
 */
// TODO どうも座標はマーカーの左上。じゃあもう送る時点で中心にすりゃいいんでね？
JNIEXPORT jint JNICALL Java_edu_dhbw_andar_ARToolkit_artoolkit_1detectmarkers
  (JNIEnv *env, jobject object, jbyteArray image, jobject transMatMonitor, jobjectArray rtnStr) {
    ARUint8         *dataPtr;
    ...
    snprintf(src,450,"%d",marker_num);
	jstring jstr = (*env)->NewStringUTF(env,src);		 // ここも実際怪しい
	const char *s = (*env)->GetStringUTFChars(env,jstr, NULL);
	(*env)->ReleaseStringUTFChars(env,jstr, s);

	// 配列の指定インデックスに文字列をセット
	(*env)->SetObjectArrayElement(env,rtnStr, 0, jstr);
    if(marker_num > 0){

		for(ii=1;ii<marker_num;ii++){
			if(ii > MAX_MARKER) break;
			//curObject->marker_width;
			i0 =(4-(marker_info+ii)->dir)%4;
			i1 = (i0+1)%4;
			i2 = (i1+1)%4;
			i3 = (i2+1)%4;
			snprintf(src,450,"%d,%5.2f,%5.2f"/*,%5.2f,%5.2f,%5.2f,%5.2f"*/,
					(marker_info+ii)->id,
					//--------- (index)X,Y -------------
					//  1   2
					//
					...
					(marker_info+ii)->pos[0] , (marker_info+ii)->pos[1] );// 4
					//(marker_info+ii)->vertex[i1][0] , (marker_info+ii)->vertex[i1][1] ,// 2
					//(marker_info+ii)->vertex[i2][0] , (marker_info+ii)->vertex[i2][1] ,// 3

			jstring jstr = (*env)->NewStringUTF(env,src);		 // ここも実際怪しい
			const char *s = (*env)->GetStringUTFChars(env,jstr, NULL);
			(*env)->ReleaseStringUTFChars(env,jstr, s);
			// 配列の指定インデックスに文字列をセット
			(*env)->SetObjectArrayElement(env,rtnStr, ii, jstr);
		}

    }
    // TODO マーカ数を返す。たぶんこれが見えた数？　idが-1は変な子だから飛ばすらしい。しらん
#ifdef DEBUG_LOGGING
