...
public class ARToolkit {
	...
	/**
	 * @return the objInfostr
	 */
	public String[] getObjInfostr() {
		return objInfostr;
	}
	/**
	 * @param objInfostr the objInfostr to set
	 */
	public void setObjInfostr(String[] objInfostr) {
		this.objInfostr = objInfostr;
	}
	
	public double[] getScreenSize() {
		double[] ret = new double [2];
		
		ret[0]=this.screenWidth;
		ret[1]=this.screenHeight;
		return ret;
	...
	public double[] getImageSize() {
		...
		ret[0]=this.imageWidth;
		ret[1]=this.imageHeight;
		return ret;
	...
	public ARToolkit(Resources res, File baseFile) {
		...
		this.res = res;
		this.setObjInfostr(new String[MAX_MARKER]);
	}
	 ...
	 */
	private native int artoolkit_detectmarkers(byte[] in, Object transMatMonitor,String[] retStr);
	...
	class DetectMarkerWorker extends Thread {
		...
		public synchronized void run() {			
			...
			while(true) {
                ...
                int currNumMakers = artoolkit_detectmarkers(curFrame, transMatMonitor,getObjInfostr()); // arToolkit.cのネイティブメソッド呼び出し
				
				if(lastNumMarkers > 0 && currNumMakers > 0) {
					//visible
				} else if(lastNumMarkers == 0 && currNumMakers > 0) {
					//detected a marker
	             	/*if(getObjInfostr()!=null){
		                for(int i=0;i<getObjInfostr().length;i++){
		                	if(getObjInfostr()[i]==null)continue;
		             		System.out.println(getObjInfostr()[i]);
		             	}
		            }*/
					notifyChange(true);
				} else if(lastNumMarkers > 0 && currNumMakers == 0) {
